...
const char DecimalFormatImpl::fgNumberPatterns[]="NumberPatterns"; // Deprecated - not used
...
void
DecimalFormatImpl::updateCurrencyPluralInfoAndPatterns(
        int32_t &changedFormattingFields, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return;
    }
...
void
DecimalFormatImpl::getEffectiveCurrency(
...
void
DecimalFormatImpl::parse(const UnicodeString& text,
                     Formattable& result,
                     ParsePosition& parsePosition) const {
    parse(text, result, parsePosition, NULL);
}

CurrencyAmount* DecimalFormatImpl::parseCurrency(
 ...
 */
void DecimalFormatImpl::parse(const UnicodeString& text,
                          Formattable& result,
...
UBool
DecimalFormatImpl::parseForCurrency(const UnicodeString& text,
                                ParsePosition& parsePosition,
                                DigitList& digits,
                                UBool* status,
                                UChar* currency) const {
 ...
 */
UBool DecimalFormatImpl::subparse(const UnicodeString& text,
 ...
 */
int32_t DecimalFormatImpl::skipPadding(const UnicodeString& text, int32_t position) const {
 ...
 */
int32_t DecimalFormatImpl::compareAffix(const UnicodeString& text,
                                    int32_t pos,
                                    UBool isNegative,
                                    UBool isPrefix,
...
UBool DecimalFormatImpl::equalWithSignCompatibility(UChar32 lhs, UChar32 rhs) const {
    if (lhs == rhs) {
        return TRUE;
    }
...
UnicodeString& DecimalFormatImpl::trimMarksFromAffix(const UnicodeString& affix, UnicodeString& trimmedAffix) {
 ...
 * @return length of input that matches, or -1 if match failure
 */
int32_t DecimalFormatImpl::compareSimpleAffix(const UnicodeString& affix,
 ...
 * pos in text.
 */
int32_t DecimalFormatImpl::skipPatternWhiteSpace(const UnicodeString& text, int32_t pos) {
 ...
 * in text.
 */
int32_t DecimalFormatImpl::skipUWhiteSpace(const UnicodeString& text, int32_t pos) {
 ...
 * in text.
 */
int32_t DecimalFormatImpl::skipUWhiteSpaceAndMarks(const UnicodeString& text, int32_t pos) {
    while (pos < text.length()) {
        UChar32 c = text.char32At(pos);
 ...
 */
int32_t DecimalFormatImpl::skipBidiMarks(const UnicodeString& text, int32_t pos) {
    while (pos < text.length()) {
 ...
 * @return length of input that matches, or -1 if match failure
 */
int32_t DecimalFormatImpl::compareComplexAffix(const UnicodeString& affixPat,
 ...
 */
int32_t DecimalFormatImpl::match(const UnicodeString& text, int32_t pos, UChar32 ch) {
 ...
 */
int32_t DecimalFormatImpl::match(const UnicodeString& text, int32_t pos, const UnicodeString& str) {
...
UBool DecimalFormatImpl::matchSymbol(const UnicodeString &text, int32_t position, int32_t length, const UnicodeString &symbol,
...
UBool DecimalFormatImpl::matchDecimal(UChar32 symbolChar,
...
UBool DecimalFormatImpl::matchGrouping(UChar32 groupingChar,
...
void
DecimalFormatImpl::setupCurrencyAffixPatterns(UErrorCode& status) {
    if (U_FAILURE(status)) {
        return;
    }
    ...
    // TODO: unfriend DecimalFormatImpl from fCurrP
...
void
DecimalFormatImpl::copyHashForAffixPattern(const Hashtable* source,
                                       Hashtable* target,
                                       UErrorCode& status) {
    if ( U_FAILURE(status) ) {
        return;
...
void
DecimalFormatImpl::deleteHashForAffixPattern()
{
