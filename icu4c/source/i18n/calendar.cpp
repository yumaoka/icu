...
void Calendar::computeTime(UErrorCode& status) {
    ...
    if (fStamp[UCAL_MILLISECONDS_IN_DAY] >= ((int32_t)kMinimumUserStamp) &&
        ...
        int32_t hourOfDay = internalGet(UCAL_HOUR_OF_DAY);
        // absolute value of hour is calculated to
        // determine if the set hour value will exceed
        // integer range when milliseconds for day is
        // calculated. An hour value upto 596 can be
        // accomodated within integer range. Here a conservative
        // estimate of 24 is used to call the appropriate function
        // Any value < 24 for hour will use the deprecated integer
        // version of the computeMillisInDay() else the double version
        // computeMillisInDayLong() is called
        if (hourOfDay < 0 ) {
          hourOfDay = hourOfDay*-1;
        }
        int32_t hour = internalGet(UCAL_HOUR);
        if (hour < 0 ) {
          hour = hour*-1;
        }
