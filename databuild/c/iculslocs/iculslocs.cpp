...
#define URES_MAX_BUFFER_SIZE 256 /* borrowed this from somewhere */
...
char packageName[URES_MAX_BUFFER_SIZE] = {0};
const char *locale = RES_INDEX; // locale referring to our index

void usage() {
  u_printf("Usage: %s [options]\n",
           PROG);
  u_printf("This program lists and optionally regenerates the locale manifests\n"
  ...
  u_printf("  -N NAME     Choose name NAME\n"
           "         (default: '%s')\n", U_ICUDATA_NAME);
  u_printf("\nNOTE: for best results, this tool ought to be linked against\n"
           "stubdata. i.e. '%s -l' SHOULD return an error with no data.\n", PROG);
}

#define ASSERT_SUCCESS(what) if(U_FAILURE(status)) { \
    u_printf("%s:%d: %s: ERROR: %s %s\n", __FILE__, __LINE__, \
             PROG, u_errorName(status), what); return 1; \
  }

void calculatePackageName() {
  packageName[0] = 0; // null
  if(strcmp(NAME,"NONE")) {
    strcpy(packageName, NAME);
    if(strcmp(TREE,"ROOT")) {
      strcat(packageName, U_TREE_SEPARATOR_STRING);
      strcat(packageName, TREE);
    }
  }
  if(VERBOSE) u_printf("packageName: %s\n", packageName);
}
 ...
 * @return 0 on "OK" (success or resource-missing), 1 on "FAILURE" (unexpected error)
 */
int localeExists(const char *loc, UBool &exists) {
  UErrorCode status = U_ZERO_ERROR;
  if(VERBOSE>1) { u_printf("Trying to open %s:%s\n", packageName, loc); }
  LocalUResourceBundlePointer aResource(ures_openDirect(packageName, loc, &status));
  exists = FALSE;
  if(U_SUCCESS(status)) {
    exists=true;
    if(VERBOSE>1) { u_printf("%s:%s existed!\n", packageName, loc); }
    return 0;
  } else if(status == U_MISSING_RESOURCE_ERROR) {
    exists=false;
    if(VERBOSE>1) { u_printf("%s:%s did NOT exist (%s)!\n", packageName, loc, u_errorName(status)); }
...
int dumpAllButInstalledLocales(int lev,
                               ...
                               LocalUFILEPointer &bf, UErrorCode &status) {
  ...
  while(U_SUCCESS(status) && ures_hasNext(bund.getAlias())) {
    ...
    if(VERBOSE>1) u_printf("dump@%d: got key %s\n", lev, key);
    if(lev==0 && !strcmp(key, INSTALLEDLOCALES)) {
      if(VERBOSE>1) u_printf("dump: skipping '%s' as it must be evaluated.\n", key);
    } else {
      printIndent(bf, lev);
      ...
      case URES_STRING:
        int32_t len=0;
        const UChar *s = ures_getString(t.getAlias(),
                                        &len,
                                        &status);
...
int list(const char *toBundle) {
  ...
  if(toBundle!=NULL) {
    if(VERBOSE) { u_printf("writing to bundle %s\n", toBundle); }
  ...
  if(VERBOSE) u_printf("\"locale\": %s\n", locale);

  LocalUResourceBundlePointer bund(ures_openDirect(packageName, locale, &status));
  ASSERT_SUCCESS("while opening the bundle");
  LocalUResourceBundlePointer installedLocales(ures_getByKey(bund.getAlias(),
                                                             INSTALLEDLOCALES,
                                                             NULL,
                                                             &status));
  ...
  for(int32_t i=0;i<count;i++) {
    subkey.adoptInstead(ures_getByIndex(installedLocales.getAlias(),
                                        i,
                                        subkey.orphan(),
                                        &status));
